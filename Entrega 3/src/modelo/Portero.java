...
public class Portero extends Jugador{
	...
	public Portero(String nombre, int precio) {
		super(nombre, precio);
	}
	...
	public void calcularPuntaje(Reporte reporte) {
		...
		int puntaje =  getPuntaje() + ((golesAnotados+penaltisAnotados)*6)+(asistencias*3)+(penaltisErrados*-2)+(tarjetasAmarillas*-1)+(tarjetasRojas*-3)+(autogoles*-2)+(golesRecibidos*4)+(penaltisDetenidos*5);
